...
def validate(
        ...
        for i, data in enumerate(data_loader):
            ...
            if PCK:
                for thr in test_PCKh.keys():
                    ckh, pckh, nckh = PCKh(
                        keypoints, test_outputs, thr=thr / 10
                    )  # dict keys are ints
            ...
            if show or save:
                ...
                figure = show_keypoints(
                    data["seq"][0][-1],
                    keypoints_pred.cpu(),
                    save=save,
                    save_fname=os.path.join(save_path, "test_" + str(i) + ".png"),
                    cmap="gray",
                    tb=(not show),
                )
                if config.wandb:
                    figures.append(wandb.Image(figure))
    ...
    return (
        torch.mean(torch.tensor(test_accuracies)),
...
def main():
    ...
    if config.wandb:
        ...
        tb_comment = "LR_%.6f_BATCH_%d_EPOCH_%d_SEQ_%d" % (
            config.lr,
            config.batch_size,
            config.epochs,
            config.seq_length,
    ...
    train_transform = [
        ...
        SequentialPoseDataset.BrightnessContrast(
            brightness=(-100, 100), contrast=(-3, 3)
        ),
    ]
    val_transform = None

    train_dataset = SequentialPoseDataset(
        video_list=config.dataset_csv,
    ...
    test_dataset = SequentialPoseDataset(
        video_list=config.dataset_test,
        video_dir=config.data_folder,
        labels_dir=os.path.join(config.data_folder, "labels_csv"),
        seq_length=config.seq_length,
    ...
    model = TLEAP(
        in_channels=3,
        out_channels=len(config.keypoints),
        seq_length=config.seq_length,
        depth=config.depth,
    ).to(config.device)
    if config.wandb:
    ...
    if config.optimizer == "sgd":
        ...
        optimizer = Adam(
            model.parameters(), lr=config.lr, amsgrad=False, weight_decay=0.01
        )
    else:  # amsgrad
        optimizer = Adam(
            model.parameters(), lr=config.lr, amsgrad=True, weight_decay=0.01
        )

    scheduler = lr_scheduler.StepLR(
        optimizer, step_size=(config.epochs // 10), gamma=0.1
    )
    ...
    fig_save_path = os.path.join(
        config.save_checkpoint, tb_comment + "_" + current_time
    )
    ...
    while epoch < config.epochs:
        ...
        val_accuracy, val_loss, val_figures, val_PCK = validate(
            model,
            criterion,
            test_loader,
            config,
            ...
            PCK=True,
            save=epoch_fig_save,
            save_path=train_fig_save_path,
        )
        print(
            "Validation loss at epoch %d: %.6f, RMSE: %.2f, PCKh@0.5: %.2f"
            % (epoch, val_loss, val_accuracy, val_PCK[5])
        )
        ...
        if config.save_checkpoint:
            model_save_path = os.path.join(
                config.save_checkpoint, tb_comment + "_" + current_time + ".model"
            )
            save_model(
                config, model, epoch, optimizer, scheduler, criterion, model_save_path
            )
    ...
    test_RSME, test_loss, test_figures, test_PCK = validate(
        model,
        criterion,
        test_loader,
        config,
    ...
    if config.wandb:
        ...
        wandb.log(
            {"Examples": test_figures, "Test RMSE": test_RSME, "Test Loss": test_loss}
        )
